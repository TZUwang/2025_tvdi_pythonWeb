...
regression_bp = Blueprint(
    "regression", # 藍圖的名稱
    __name__,
    url_prefix="/regression", # 藍圖的網址
    template_folder="../static", #當使用者訪問 /regression/regression_index 
)
...
def regression_predict():
    ...
    # 取得使用者輸入的房間數
        ...
        sample_size = 200
        feature_idx = 2
        X = housing.data[:sample_size,feature_idx].reshape(-1,1) #特徵 #reshape(-1,1) 將二維陣列轉換為一維 因為線性迴歸模型只接受一維數據
