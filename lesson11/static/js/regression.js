...
function addPredictionPoint(x, y){ //專門新增新的預測點
    chart.data.datasets
    //先移除舊的預測點
    ...
    existingDatasets.push({
        label: '您的預測',
        data: [{ x: x, y: y }],
        backgroundColor: '#FFc107', //點的顏色
        borderColor: '#FFc107 ', //邊框顏色
        pointRadius: 12, //點的大小
        pointHoverRadius: 15,//滑鼠懸停時的點的大小
        pointStyle: 'star', //點的形狀
        borderWidth: 3,
    })
...
function updateMetrics(metrics) {
    ...
    if (r2Value > 0.7) {
        r2Element.style.color = '#28a745';
    } else if (r2Value > 0.4) {
        r2Element.style.color = '#ffc107';
    } else {
